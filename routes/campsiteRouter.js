...
campsiteRouter
  ...
  .post(authenticate.verifyUser, (req, res) => {
    res.statusCode = 403;
    res.end(
      `POST operation not suppored on /campsites/${req.params.campsiteId}/comments/${req.params.commentId}`
    );
  })
  .put(authenticate.verifyUser, (req, res, next) => {
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          const userIsAuthor = campsite.comments
          ...
          if (userIsAuthor) {
            ...
            err = new Error("You are not authorized to delete ths comment!");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
